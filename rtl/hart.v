...
`endif
  ...
  //allow the IF/ID queue to fetch ahead until it is full
  wire ifetch_ready = (~icache_busy_q) & (~if_id_buffer_full);
...
`ifdef PERF_COUNTERS
  ...
  reg [63:0] perf_retired;
  reg [63:0] perf_icache_reqs;
  reg [63:0] perf_icache_hits;
  reg [63:0] perf_icache_misses;
  reg [63:0] perf_icache_busy_cycles;
  reg [63:0] perf_dcache_reqs;
  reg [63:0] perf_dcache_hits;
  reg [63:0] perf_dcache_misses;
  reg [63:0] perf_dcache_busy_cycles;
  reg        perf_dcache_busy_q;
  reg        perf_printed;

  wire perf_retire_fire = (~i_rst) & wb_is_retiring;
  wire perf_icache_req_fire = ifetch_req_accepted;
  wire perf_dcache_req_active = cpu_dmem_ren | cpu_dmem_wen;
  wire perf_dcache_req_fire = perf_dcache_req_active & ~perf_dcache_busy_q;

  task perf_print_counters;
    begin
      $display("PERF cycles                 : %0d", perf_cycles);
      $display("PERF retired                : %0d", perf_retired);
      if (perf_retired != 0)
        $display("PERF CPI                    : %0f", perf_cycles / (1.0 * perf_retired));
      else
        $display("PERF CPI                    : invalid");
      $display("PERF icache requests        : %0d", perf_icache_reqs);
      $display("PERF icache hits            : %0d", perf_icache_hits);
      $display("PERF icache misses          : %0d", perf_icache_misses);
      $display("PERF icache busy cycles     : %0d", perf_icache_busy_cycles);
      $display("PERF dcache requests        : %0d", perf_dcache_reqs);
      $display("PERF dcache hits            : %0d", perf_dcache_hits);
      $display("PERF dcache misses          : %0d", perf_dcache_misses);
      $display("PERF dcache busy cycles     : %0d", perf_dcache_busy_cycles);
    end
  endtask
  ...
  always @(posedge i_clk) begin
    if (i_rst) begin
      perf_cycles <= 64'd0;
      perf_retired <= 64'd0;
      perf_icache_reqs <= 64'd0;
      perf_icache_hits <= 64'd0;
      perf_icache_misses <= 64'd0;
      perf_icache_busy_cycles <= 64'd0;
      perf_dcache_reqs <= 64'd0;
      perf_dcache_hits <= 64'd0;
      perf_dcache_misses <= 64'd0;
      perf_dcache_busy_cycles <= 64'd0;
      perf_dcache_busy_q <= 1'b0;
      perf_printed <= 1'b0;
    end else begin
      perf_cycles <= perf_cycles + 64'd1;
      perf_dcache_busy_q <= dcache_busy;

      if (perf_retire_fire)
        perf_retired <= perf_retired + 64'd1;

      if (perf_icache_req_fire) begin
        perf_icache_reqs <= perf_icache_reqs + 64'd1;
        if (icache_busy)
          perf_icache_misses <= perf_icache_misses + 64'd1;
        else
          perf_icache_hits <= perf_icache_hits + 64'd1;
      end

      if (icache_busy)
        perf_icache_busy_cycles <= perf_icache_busy_cycles + 64'd1;

      if (perf_dcache_req_fire) begin
        perf_dcache_reqs <= perf_dcache_reqs + 64'd1;
        if (dcache_busy)
          perf_dcache_misses <= perf_dcache_misses + 64'd1;
        else
          perf_dcache_hits <= perf_dcache_hits + 64'd1;
      end

      if (dcache_busy)
        perf_dcache_busy_cycles <= perf_dcache_busy_cycles + 64'd1;

      if (wb_is_halting & perf_retire_fire & ~perf_printed) begin
        perf_printed <= 1'b1;
        perf_print_counters();
      end
    end
