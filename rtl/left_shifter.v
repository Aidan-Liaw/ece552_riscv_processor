 ...
 (
    ...
    generate
        ...
        for (idx = DATA_WIDTH - 1; idx >= 0; idx = idx - 1) begin : left_shifter_gen
            // 1. op2 can equal 0xFFFF in theory, which would be considered negative, and cause problems, so the unsigned function stops this from being one
            ...
            assign o_result[idx] = ((idx - op2[SHIFT_WIDTH - 1:0]) & ({1'b1, {(DATA_WIDTH - 1){1'b0}}})) != ({1'b1, {(DATA_WIDTH - 1){1'b0}}})
                                   ? op1[idx - op2[SHIFT_WIDTH - 1:0]]
                                   : 1'b0;
        end
    endgenerate
